...
$sOrthologList = "filtered_genespace.orthogroups.txt";
$sOutgroupList = "outgroups.txt"; //use these sequences directly, instead of exporting from fasta/gff pairs
$sGenomeDef = "genomes_F5.txt";
$arrGenomes = array();
$sOutputPrefix = "output_F5/ret";
$nTotalParts = 1;
$nThisPart = 0; //index starts from 0
$nMissingCutoff = 0.1; //at most allow 50% missing
...
for($nOrth=0;$nOrth<count($arrOrthologs);$nOrth++ ) {
	...
	foreach( array_keys($arrOutgroupProt) as $sOutgroupSp ) {
		...
		if ($sIDString == 'NA') {
			echo("Warning: gene not found in cds file for $sOutgroupSp, skipped\n");
			continue;
		}
