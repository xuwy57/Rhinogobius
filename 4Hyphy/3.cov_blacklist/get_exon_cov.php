...
$sGenomeFai = "$sGenome.fai";
exec("awk -v OFS='\t' {'print $1,$2'}  $sGenome.fai > $sOutSpDIR/genomefile.txt");

$command ="bedtools sort -faidx $sGenomeFai -i $sExonBed > $sExonSortedBed";
exec($command);

exec("if [ -e $sHistogramDone ]; then echo $sHistogramDone exists, skip; else samtools view -u -q $nMinMapQ $sBAM | bedtools coverage -hist -a $sExonSortedBed -b stdin -sorted -g $sOutSpDIR/genomefile.txt > $sHistogram && touch $sHistogramDone; fi");
