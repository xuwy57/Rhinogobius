...
#for(nR in 1) {
  ...
  oTree <- tryCatch( { read.tree(text=sTree)} , error=function(e) {cat("Failed to parse tree\n", sTree,"\n"); return(F);} );
  if ( class(oTree) == "logical") {
    next;
  }
  ...
  if (nMarkStyle == 'codeml') {
    ...
    #print("mark codeml style\n");
    bErr <- F;
    for(nForeground in 1:length(arrForegroundClades) ) {
      sForegroundClade <- arrForegroundClades[nForeground];
      bMarkChildren <- arrMarkForegroundChildren[nForeground];
      cat("mark: ", sForegroundClade, 'Markchildren:', bMarkChildren,"\n");
      arrDescTips <- fnGetAllDescendantTips(sForegroundClade , oClades);
      print(arrDescTips);
      if (is.null(arrDescTips) ) {
        if (is.null(arrDescTips)) {
          bErr <-T;
	  oTree$tip.label[oTree$tip.label == sForegroundClade ] <- paste(sForegroundClade, '#1', sep='');
	  print("null desc\n");
	  break;
        }
        break;
