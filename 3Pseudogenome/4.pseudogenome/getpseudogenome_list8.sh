...
mkdir ./testaln_list8
php getAlignmentAllowN_GATK.php -n yes -q 1 -D no -w 1000000000 -c list8.txt -R charRequirements.txt -s 1 -l 10 -d 1 -N no -r 0.5 -o ./testaln_list8 -I yes -b no -M yes
[ $? -ne 0 ] &&  exit 1 ;



arrInput=( `cut -f1 list8.txt` )
#arrInput=( `cut -f1 list1taxa.txt` )
sRefGenome=`cut -f2 list8.txt | head -n 1`
...
( php phy2fasta_list8.php -R $sRefGenome -L ${i} -o ${i}_pseudogenome.fasta ; fastahack -i ${i}_pseudogenome.fasta ) &
