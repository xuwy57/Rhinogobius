	...
	for ($i=0;$i< $nLenContig;$i++) {
			...
			foreach($arrBCFUtils as $sTaxon => $oBCFTool) {
				...
				if ($sAltBase != $sRefBase) {
					$arrDepthForAlleles = explode(",", $arrPos1["AD"]);
					if (count($arrDepthForAlleles) ==2) {
						if (($arrDepthForAlleles[0] + $arrDepthForAlleles[1] ) == 0) {
							$bAltIsMajor = false;							
						} else {
				...
				$nVARQUAL = array_key_exists('RGQ', $arrPos1)? $arrPos1['RGQ'] : $arrPos1[2]; //if is ref hom set RGQ as qual, if is variant set variant qual as qual 
				if ($bArbitraryPutMajorRead) {
					...
					$arrCurrBase[$sTaxon] =  (($nVARQUAL >= $nMinQual && $arrPos1["DP"]>= $nRequiredMinDepth)? $sBaseToPut : 'N');

				} else {
					$arrCurrBase[$sTaxon] =  (($nVARQUAL >= $nMinQual && $arrPos1["DP"]>= $nRequiredMinDepth && $bAltIsMajor)? $sAltBase : 'N');
				}
				//print_r($arrPos1);
