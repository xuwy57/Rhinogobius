...
$sCloseOutgroup = "/data/projects/zma/Rhinogobius_spp/DNA/bam/Z6_R-giurinus_Zhejianglongquanshizhulongzhen.bam";
$sFarOutgroup = "Tridentiger_barbatus";
$sOut = "SV.polarizedRT.out.txt";
...
while(false !== ($sLn = fgets($hVCF) )) {
  ...
  $bAllNA = true;                    // 新增：标记是否全部为 NA
    ...
    // 修改关键判断：不再检查 close outgroup 的 mask
        if ($nGeno == -1) {
            $arrPolarizedSamples[] = 'NA';
        } else if ($nGeno == 1) {           // heterozygous
