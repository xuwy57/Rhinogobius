...
#high impact
clade2 <- read.table("high_clade2_vs_amphidromous.txt", 
                     header = TRUE, sep = "\t", stringsAsFactors = FALSE)
clade5 <- read.table("high_clade5_vs_amphidromous.txt", 
                     header = TRUE, sep = "\t", stringsAsFactors = FALSE)
clade7 <- read.table("high_clade7_vs_amphidromous.txt", 
                     header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
write.table(df_selected, "/fast3/group_crf/home/b20xuwy57/Rhinogobius_spp/plot/fig4/final_fig4data.csv", sep = ",", col.names = T, row.names = F, quote = F)
...
# ------------------------------
# 1. 读取三个clade的high impact结果，筛选p<0.05且change>0的基因
# ------------------------------
clade2 <- read.table("high_clade2_vs_amphidromous.txt",
                     header = TRUE, sep = "\t", stringsAsFactors = FALSE)
clade5 <- read.table("high_clade5_vs_amphidromous.txt",
...
# 添加基因符号（假设fnAddGeneSymbols函数已定义）
clade2 <- fnAddGeneSymbols(clade2)
clade5 <- fnAddGeneSymbols(clade5)
clade7 <- fnAddGeneSymbols(clade7)

# 重命名各数据框的列（除gene_id外加上后缀）
clade2_renamed <- clade2 %>% rename_with(~ ifelse(.x == "gene_id", .x, paste0(.x, "_clade2")))
clade5_renamed <- clade5 %>% rename_with(~ ifelse(.x == "gene_id", .x, paste0(.x, "_clade5")))
clade7_renamed <- clade7 %>% rename_with(~ ifelse(.x == "gene_id", .x, paste0(.x, "_clade7")))
...
# ------------------------------
# 2. 读取snpEff的物种-基因计数矩阵
# ------------------------------
# ... 前面的代码保持不变，直到 sorted_data 生成 ...

# 读取 snpeff_data 并确保数值列是数值型
snpeff_data <- read.table("/fast3/group_crf/home/b20xuwy57/Rhinogobius_spp/snpEff/snpeff_0413/high_gene_species_counts.txt",
...
snpeff_data$"Node IDs" <- rownames(snpeff_data)
snpeff_data <- snpeff_data %>% select("Node IDs", everything())
snpeff_data[, -1] <- lapply(snpeff_data[, -1], function(x) as.numeric(as.character(x)))

# 给 sorted_data 添加两列：
# - all_non_na: 该基因在所有物种中是否完整
# - has_symbol: 该基因是否有有效的基因符号（以 clade7 为准）
sorted_data <- sorted_data %>%
  rowwise() %>%
  mutate(
    all_non_na = {
      gene <- gene_id
      if (gene %in% colnames(snpeff_data)) {
        col_vals <- snpeff_data[[gene]]
        all(!is.na(col_vals))
      } else {
        FALSE
      }
    },
    has_symbol = (!is.na(genesymbols_clade7) & genesymbols_clade7 != "")
  ) %>%
  ungroup()

# 筛选同时满足“完整”和“有symbol”的基因，按 mean_change 降序，取前30
df_top30_complete_with_symbol <- sorted_data %>%
  filter(all_non_na == TRUE, has_symbol == TRUE) %>%
  arrange(desc(mean_change)) %>%
  head(30)

# 如果不足30个，给出警告
if (nrow(df_top30_complete_with_symbol) < 30) {
  warning(paste("只找到", nrow(df_top30_complete_with_symbol), 
                "个同时满足数据完整且有symbol的基因，不足30个。"))
}

# 提取基因ID与symbol的对应表
gene_map <- df_top30_complete_with_symbol[, c("gene_id", "genesymbols_clade7")]

# 从 snpeff_data 中提取这些基因的列，并将列名替换为 symbol
genes_to_keep <- gene_map$gene_id
cols_to_keep <- c("Node IDs", intersect(genes_to_keep, colnames(snpeff_data)))
df_selected <- snpeff_data[, cols_to_keep, drop = FALSE]
